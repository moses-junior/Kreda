	...
	if (file_exists($csvFile)) {
		...
		while (($data = fgetcsv($handle, 2000, ";")) !== FALSE) {
			...
			for ($x=0; $x < count($data); $x++) {
				if ($start == 0 && $hasTitle == true) {
					...
					switch ($headers[$x]) {
						...
						//case "Schueler_Name":					$sql_insert_headers[]="name"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); $schueler_name=$data[$x]; break;
						//case "Schueler_Vorname":				$sql_insert_headers[]="vorname"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); $schueler_name.=", ".$data[$x]; break;
						//case "Schueler_Klasse":					$k_id=$_POST[str_replace(' ','',$data[$x])]; if (isset($k_id) and $k_id!=0) { $sql_insert_headers[]="klasse"; $sql_insert_values[]=injaway($k_id); $klasse_zugeordnet=true; } break;
						//case "Schueler_Geschlecht":				$sql_insert_headers[]="maennlich"; if ($data[$x]=="w") $sql_insert_values[]=0; else $sql_insert_values[]=1; break;
						//case "Schueler_Klasse":					$schueler_name.=", ".$data[$x]; break;
						//case "Schueler_Strasse":				$sql_insert_headers[]="strasse"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Plz":					$sql_insert_headers[]="plz"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Wohnort":				if ($headers[$x+1]=="Schueler_Ortsteil" and !empty($data[$x+1]))
																	$ort=$data[$x]." OT ".$data[$x+1];
																else
																	$ort=$data[$x];
																$sql_insert_headers[]="ort"; $sql_insert_values[]=apostroph_bei_bedarf($ort); break;
						case "Schueler_Geburtsdatum":			$sql_insert_headers[]="geburtstag"; $sql_insert_values[]=apostroph_bei_bedarf(datum_punkt_zu_strich($data[$x])); break;
						case "Schueler_Geburtsort":				$sql_insert_headers[]="geburtsort"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_allgemeine_Bemerkungen": $sql_insert_headers[]="bemerkungen"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Krankenkasse":			$sql_insert_headers[]="krankenkasse"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Staatsangehoerigkeit":	$sql_insert_headers[]="staatsangehoerigkeit"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Konfession":				$sql_insert_headers[]="konfession"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Aufnahme_am":			$sql_insert_headers[]="aufnahme_am"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Abgang_am":				$sql_insert_headers[]="abgang_am"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_abgebende_Schule_ID":	$sql_insert_headers[]="abgebende_schule"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_aufnehmende_Schule_ID":	$sql_insert_headers[]="aufnehmende_schule"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Schliessfach_Schluesselnummer": $sql_insert_headers[]="schliessfach_schl_nr"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Schliessfachnummer":		$sql_insert_headers[]="schliessfachnummer"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Schueler_Fotoerlaubnis":			$sql_insert_headers[]="fotoerlaubnis"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter1_Name":			$sql_insert_headers_sb[0][]="surname"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter1_Vorname":		$sql_insert_headers_sb[0][]="forename"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter1_Strasse":		$sql_insert_headers_sb[0][]="adress"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter1_Plz":			$sql_insert_headers_sb[0][]="postal_code"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter1_Wohnort":		if ($headers[$x+1]=="Sorgeberechtigter1_Ortsteil" and !empty($data[$x+1]))
																	$ort=$data[$x]." OT ".$data[$x+1];
																else
																	$ort=$data[$x];
																$sql_insert_headers_sb[0][]="city"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($ort); break;
						case "Sorgeberechtigter1_Geschlecht":	$sql_insert_headers_sb[0][]="male"; if ($data[$x]=="w") $sql_insert_values_sb[0][]=0; else $sql_insert_values_sb[0][]=1; break;
						case "Sorgeberechtigter1_Titel":		$sql_insert_headers_sb[0][]="title"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter2_Name":			$sql_insert_headers_sb[1][]="surname"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter2_Vorname":		$sql_insert_headers_sb[1][]="forename"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter2_Strasse":		$sql_insert_headers_sb[1][]="adress"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter2_Plz":			$sql_insert_headers_sb[1][]="postal_code"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						case "Sorgeberechtigter2_Wohnort":		if ($headers[$x+1]=="Sorgeberechtigter2_Ortsteil" and !empty($data[$x+1]))
																	$ort=$data[$x]." OT ".$data[$x+1];
																else
																	$ort=$data[$x];
																$sql_insert_headers_sb[1][]="city"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($ort); break;
						case "Sorgeberechtigter2_Geschlecht":	$sql_insert_headers_sb[1][]="male"; if ($data[$x]=="w") $sql_insert_values_sb[1][]=0; else $sql_insert_values_sb[1][]=1; break;
						case "Sorgeberechtigter2_Titel":		$sql_insert_headers_sb[1][]="title"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Bankname":				$sql_insert_headers[]="bank"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Bankleitzahl":			$sql_insert_headers[]="blz"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Kontonummer":			$sql_insert_headers[]="kto_nr"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Kontoinhaber":			$sql_insert_headers[]="kto_inhaber"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Iban":					$sql_insert_headers[]="iban"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Bic":					$sql_insert_headers[]="bic"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Benutzername":			$sql_insert_headers[]="username"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						//case "Schueler_Passwort":				$sql_insert_headers[]="passwort"; $sql_insert_values[]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Email":				$sql_insert_headers_sb[0][]="user_email"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Telefon1":			$sql_insert_headers_sb[0][]="tel1"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Telefon2":			$sql_insert_headers_sb[0][]="tel2"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Telefon3":			$sql_insert_headers_sb[0][]="tel3"; $sql_insert_values_sb[0][]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Telefon4":			$sql_insert_headers_sb[1][]="tel1"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Telefon5":			$sql_insert_headers_sb[1][]="tel2"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						case "Kommunikation_Telefon6":			$sql_insert_headers_sb[1][]="tel3"; $sql_insert_values_sb[1][]=apostroph_bei_bedarf($data[$x]); break;
						
						case "Sorgeberechtigter_Postempfaenger":$sql_insert_value_post=apostroph_bei_bedarf($data[$x]); break;
			...
			if ($s_nr>0) {
						...
						for($e=0; $e<=1; $e++) {
							...
							for($k=0;$k<count($sql_insert_headers_sb[$e]); $k++) {
								switch ($sql_insert_headers_sb[$e][$k]) {
									...
									case "adress": $suchfeld["adress"]=trim($sql_insert_values_sb[$e][$k]); break;
							...
							} // Falls der Sorgeberechtigte eingetragen ist
				...
				$nicht_aktualisierte_schueler[]=$schueler_name." (FS-SNr: ".$s_nr.")";
