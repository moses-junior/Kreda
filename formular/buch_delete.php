...
if (!proofuser("buch",$_GET["id"]))
	die("Sie sind hierzu nicht berechtigt.");

if($_GET["bestaetigen"]=="ja") {
   $deleter=del_array2echo(delete_db_object("buch", array(injaway($_GET["id"])), $pfad, false), "sql");
    ...
    echo '<body><div class="inhalt"><form action="'.$pfad.'formular/buch_delete.php?bestaetigen=ja&amp;id='.$_GET["id"].'" method="post">
        <p>Das Buch wird endg&uuml;ltig (zusammen mit allen ihm zugeordneten Daten) gel&ouml;scht.</p>';
    echo del_array2echo(delete_db_object("buch", array(injaway($_GET["id"])), $pfad, false), "info");
